...
const Following: NextPage = () => {
  ...
  const Row = ({ index, style }: { index: number; style: CSSProperties }) => {
    return following?.getFollow.data[index] != null ||
      following?.getFollow.data[index] != undefined ? (
      <Author
        style={style}
        isFollow={
          me?.me
            ? me?.me.follow.includes(following?.getFollow.data[index].id)
            : false
        }
        key={following?.getFollow.data[index].id}
        id={following?.getFollow.data[index].id}
        src={following?.getFollow.data[index].profileImage}
        name={following?.getFollow.data[index].userName}
      />
    ) : (
      <div
          ...
          {following?.getFollow && (
            <AutoSizer>
              {({ height, width }: { height: number; width: number }) => (
                <InfiniteLoader
                  isItemLoaded={(index) =>
                    !loading && index < following?.getFollow.data.length
                  }
                  itemCount={following?.getFollow.count}
